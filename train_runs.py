...
if __name__ == '__main__':
    ...
    parser.add_argument('--method', type=str, help='whether to choose acc or auprc or both')
    ...
    run_name = "_".join([args.loader, args.dataset, args.method, "MC2.json"])
    print(run_name)
    ...
    # temp_settings = {}
    # temp_settings['min_tau'] = 0.07
    # temp_settings['max_tau'] = 0.8
    # temp_settings['t_max'] = 5
    # temp_settings = None
    ...
    {
        "min_tau": pyhopper.float(0,0.3, "0.5f"),
        "max_tau": pyhopper.float(0.5,1, "0.5f"),
        "t_max"  : pyhopper.float(5,20, "0.5f"),
        # "t_max"  : pyhopper.float(1e1,5,"0.1g")
    }
    )
    # temp_settings = search.run(objective, "maximize", "20m", n_jobs=1)
    try:
        temp_settings = search.run(objective, "maximize", steps=20, n_jobs=1)
    except:
        temp_settings = None
